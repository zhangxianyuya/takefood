            ...
            <el-table :data="tableData" style="width: 100%" class="table" border>
                ...
                <el-table-column label="操作" width="120" align="center">
                    <template slot-scope="scope">
                        <el-button type="primary" @click="handleConfirm(scope.row)">确认收货</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
export default {
    ...
    methods: {
        ...
        handleConfirm(row) {
    console.log("确认收货请求发送前，订单ID:", row.order_id);
    // 发送确认收货请求，并传递订单ID
    this.$axios.post("/api/user/confirm_receipt", { order_id: row.order_id }).then((res) => {
        console.log("确认收货请求响应:", res.data);
        if (res.data.status == 200) {
            // 刷新订单列表
            this.getdata();
        }
